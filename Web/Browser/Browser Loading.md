...
(라우터가 DNS 서버 기능을 가진다. 라우터 에는 웹서버의 IP주소가 없으므로 라우터에서 다시 DNS로 질의 한다.)
...
도메인도 종류를 나눌 수 있다.
	...
	도메인 레벨 중 가장 높은 단계에 있는 도메인이다. 보통 국가나 특정 주제를 나타낸다. com, net, kr
	- Domain Name
	도메인명은 도메인의 고유한 이름을 의미한다. 여기서는 naver가 도메인명이 된다.
	- SLD(Second-Level Domain)
	호스트 명, 서브 도메인이라고 하며, 루트 도메인 앞에 존재한다. 용도를 구분하기 위해 서브 도메인을 사용한다. m, www, dev 등등
[[Network#^7823ff]]

- Path
`/`파일의 경로를 의미한다. 디렉터리 구조 처럼 생각할 수 있으며 해당 기준으로 리소스를 구성할 수 있다.

주어진 URL을 해석하면 HTTPS 프로토콜을 이용하여 d2.naver.com에 해당하는 서버에 helloworld/59361 디렉토리의 리소스를 요청한다는 의미다.

### DNS 조회
입력한 URL을 바탕으로 서버의 IP주소를 조회해야 하는데 이때 DNS조회를 사용한다.
URL의 호스트명을 이용해서 DNS에 저장한 고유한 IP주소를 찾아낸다.
이때 DNS는 각 계층마다 캐시를 가지고 있고 이를 조회하며 이때 캐시에 없다면 DNS 서버를 재귀적으로 조회하며 해당 주소를 찾아낸다.

### 서버와 TCP, TLS연결
DNS에서 찾은 서버 IP주소를 통해 접근하게 된다. 이때 브라우저와 서버간의 TCP 연결이 필요하다. 여기서 HTTPS 프로토콜인 경우 추가로 TLS 핸드쉐이크 과정을 더 거친다.

먼저 TLS 핸드쉐이크 과정을 거치기전에 TCP통신을 통해 클라이언트와 서버가 먼저 핸드 쉐이크 과정을 거친다.

연결이 성공되면 TLS 핸드 쉐이크 과정도 거친다. 매번 연결할 때마다 진행하면 비효율적이기 때문에 최초 한번 연결과정을 진행하고 Session ID를 가진다.
이때 공개 키, 대칭 키 방식이 적용된다. 먼저 공개 키 방식으로 핸드 쉐이크 과정을 수행한다.
클라이언트는 공개 키를 받고 인증서를 받아 신뢰할만한지 확인한다. 그리고 

인증이 되면 이후 세션키를 설정할 수 있게 된다.
세션키가 생성되고 나서는 별도의 인증없이 세션 키를 통해 메시지를 암호화하여 주고 받는다.
