...
부동 소수점 표현은 IEEE 754표준을 따르며 소수를 표현하는 방식이다. 자바스크립트는 64비트 부동 소수점 형식을 따른다.
소수점 부분을 고정적으로 지정하여 정수, 소수 부분을 메모리에 저장하지 않고 유동적으로 지정하여 저장하는 방식이다.

실수를 저장할 때 부호, 지수부, 가수부로 나누어 저장한다.
지수부는 1bit, 지수부는 11bit, 가수부는 52bit로 이루어진다.

![](https://blog.kakaocdn.net/dn/dvH7hH/btqZcsZagz7/LWPXsoCKtOjW2bUt0R5Pp0/img.png)

단순히 정수, 소수를 나누어 저장하지 않고 일정 규칙을 통해 변환과정을 통해서 저장한다. 규칙은 2진수로 변환한 뒤 2진수의 정수부가 한자릿수가 되도록 지수변환을 해준뒤 저장한다.

예를 들어 118.625라는 숫자를 저장할 때 양수이므로 부호에는 0을 저장하고 정수, 소수부를 2진로 변환한다. 
```
# 정수부 변환
118
= 1110110(2)
...
그 뒤 정수부가 한자리가 되도록 변환해준다.
```
1110110.101 → 1.110110101 x 2^6
```

지수에 바이어스 값 1023을 더하고 지수부 비트에 6 + 1023값 1029 => 10000000101(2)을 넣고 가수부에 1110110101를 저장한다.
바이어스는 고정된 127값이며 이를 더하는 이유는 음수가 될 케이스에 대비하기 위해서다. 따라서 지수에 127보다 작으면 음수, 크면 양수로 구분된다.

부동 소수점의 장점은 큰 범위의 값을 표현할 때 좋지만, 어쨋든 무한한 소수나 큰 정수의 경우 표현할 수 있는 범위의 제한이 있다.
...
정수의 범위는 가수부가 53비트이기 때문에 표현할 수 있는 정수는 -2^53-1에서 2^53 -1 까지 이다. 

그래서 그 이상 또는 이하의 정수는 표현이 되지 않기 때문에 오차가 발생한다.
```js
console.log(9007199254740991 + 1); // 9007199254740992 (정확)
console.log(9007199254740991 + 2); // 9007199254740992 (오차 발생)
...
해결 방법은 BigInt를 활용해서 범위 바깥의 정수를 표현하고 계산할 수 있다.
