...
> 프로미스도 then메서드를 가지며 이를 만족하기 때문에 thenable에 개념에 포함되며, thenable하다고 표현한다.
...
# Async/Await
---
비동기 처리를 동기 처리처럼 구현할 수 있게 해주는 문법이다. async/await을 사용하면 비동기 코드의 가독성도 좋아진다.
async/await 구문은 프로미스를 기반으로 동작한다. 

then/catch/finally 후속처리 메서드를 사용할 필요가 없어 콜백 함수를 활용하지 않아도돼 동기 처리처럼 프로미스를 사용할 수 있다.

try/catch 구문과 같이 사용할 수 있고 일반 비동기 함수에서 try/catch구문의 에러 전파문제 없이 에러처리가 가능해진다.
...
await 키워드는 프로미스가 settled 상태가 될 때까지 대기하다가 프로미스가 처리한 reolve한 결과를 반환한다. 그리고 settled상태가 될 때까지 다음 코드를 일시중지 시킨다.
하지만 기다리는 동안에 다른 스크립트 실행이나 이벤트 처리는 할 수 있다.

await은 thenable 객체를 받는다. then 메서드를 가지고 있는 thenable 객체면 프로미스가 아니더라도 이를 받아서 사용할 수 있다. 여기에는 fetch, Promise.all의 반환 값등 thenable한 객체면 호환이 된다.
[[Async, Promise#^ea7383]]
